...
def mapFunc7(userdata):
    ...
    for line in inp:
        ...
        for word in wordList[1:]:
            relaxed = word.strip().lower().lstrip("(").rstrip(")")
            if (len(relaxed) > 0):
                tuple = (relaxed, lineNum)
                outp.append(tuple)
    return outp
...
map7 = userdataDir.flatMap(mapFunc7).groupByKey().map(lambda x: (x[0], list(x[1])))
reduce7 = map7.map(reduceFunc7)
output7 = reduce7.filter(lambda x: len(x[1]) > 0).map(lambda x: x[0] + ", " + str(x[1]))
